...
### UI changes
When parsing a file, it would be nice if we could visualize the data that is produced. For example, when parsing a python file, we could visualize the AST of the file. When parsing a PDF file, we could visualize the markdown of the file. The process of parsing visualization would be really helpful to understand the data that is produced and debug/improve the parsing process.